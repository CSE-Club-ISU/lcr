...
# Parse both identity and token from response
PUBLISH_IDENTITY=$(echo "$IDENTITY_RESPONSE" | grep -o '"identity":"[^"]*' | cut -d'"' -f4)
PUBLISH_TOKEN=$(echo "$IDENTITY_RESPONSE" | grep -o '"token":"[^"]*' | cut -d'"' -f4)

if [ -z "$PUBLISH_TOKEN" ] || [ -z "$PUBLISH_IDENTITY" ]; then
...
# The token from the publish step is used for seeding (already valid).
TOKEN="${PUBLISH_TOKEN}"
