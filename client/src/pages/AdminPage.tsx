...
import { X, Check, Copy, Pencil, Trash2 } from 'lucide-react';
...
function problemToDraft(p: Problem): EditDraft {
  return {
    ...
    methodSignatures:  p.methodSignatures  ?? '',
...
function EditModal({ problem, onClose }: { problem: Problem; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-bg rounded-md w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col"
        style={{ border: '1px solid var(--color-hairline-strong)', boxShadow: 'var(--shadow-hero)' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 shrink-0" style={{ borderBottom: '1px solid var(--color-hairline)' }}>
          <span style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: 18 }}>Edit problem</span>
          <button
            onClick={onClose}
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-6 py-4 shrink-0" style={{ borderTop: '1px solid var(--color-hairline)' }}>
          <button onClick={onClose} className="btn-ghost">Cancel</button>
          <button onClick={handleSave} disabled={saving} className="btn-editorial" style={{ opacity: saving ? 0.5 : 1 }}>
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-[11.5px] text-text-muted bg-transparent cursor-pointer hover:text-text transition-colors"
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-[11.5px] text-text-muted bg-transparent cursor-pointer hover:text-red transition-colors"
                  style={{ border: '1px solid var(--color-hairline)' }}
                >
    ...
    <div className="flex flex-col gap-5 max-w-[720px]">
        ...
        <div className="mt-2 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center gap-1.5 px-3 py-1 rounded-sm text-[12px] cursor-pointer text-text-muted hover:text-text transition-colors"
              style={{ border: '1px solid var(--color-hairline-strong)' }}
            >
              {schemaCopied ? <><Check size={11} /> Copied</> : <><Copy size={11} /> Copy LLM prompt</>}
            </button>
          </div>
      ...
      {parsed && (
        <div className="flex flex-col gap-4">
          ...
          <button className="btn-editorial self-start" onClick={handleSubmit}>
      ...
      {submitted && (
        <div className="text-green text-sm font-semibold inline-flex items-center gap-2">
          <Check size={14} /> {submitCount} problem{submitCount !== 1 ? 's' : ''} submitted. Check the Problems tab.
        </div>
      )}
    </div>
  );
...
export default function AdminPage() {
  ...
  return (
    <div className="enter-fade flex flex-col gap-8">
      <header className="pb-5" style={{ borderBottom: '1px solid var(--color-hairline)' }}>
        <div className="label-eyebrow mb-2">Admin</div>
        <h1 className="display-title" style={{ fontSize: 'clamp(32px, 4vw, 44px)' }}>
          Workshop.
        </h1>
        <p className="text-text-muted text-[13px] mt-2">Manage problems and approve submissions.</p>
      </header>

      {/* Editorial tab strip */}
      <div className="flex items-center gap-6" style={{ borderBottom: '1px solid var(--color-hairline)' }}>
